...
module.exports = function compile(options) {
  ...
  var middlewareConfig = options.middleware ||
    ConfigLoader.loadMiddleware(appRootDir, env);
  var middlewareInstructions =
    buildMiddlewareInstructions(appRootDir, middlewareConfig);
...
function buildMiddlewareInstructions(rootDir, config) {
  ...
  phasesNames.forEach(function(phase) {
    ...
    Object.keys(phaseConfig).forEach(function(middleware) {
      ...
      var factoryFile = start !== './' && start !== '..' ?
        middleware :
        path.resolve(rootDir, middleware);

      // TODO(bajtos) support array of configs

      var middlewareConfig = cloneDeep(phaseConfig[middleware]);
      middlewareConfig.phase = phase;

      middlewareList.push({
        factoryFile: require.resolve(factoryFile),
