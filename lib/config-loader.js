...
function mergeMiddlewareConfig(target, config, fileName) {
  if (config.phases) {
    throw new Error('Cannot apply ' + fileName + ': phases cannot be altered.');
  }
  for (var m in config.middleware) {
    var err;
    if (config.middleware[m].phase) {
      err = 'middleware phase cannot be altered.';
    } else {
      err = mergeObjects(
        target.middleware[m].config,
        config.middleware[m].config);
    }
    if (err) {
      throw new Error('Cannot apply ' + fileName + ' to `'  + m + '`: ' + err);
    }
  }
}
